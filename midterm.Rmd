...
```{r}
set.seed(123)
x1 <- model.matrix(log_antibody ~ ., dat1)[, -1]
y1 <- dat1$log_antibody
...
pred_gam <- predict(gam.fit$finalModel, newdata = dat2)
sqrt(mean((pred_gam - y2)^2))
```

```{r}
resample = resamples(list(lasso = lasso.fit, gam = gam.fit))
summary(resample)
```
