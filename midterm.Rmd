...
ggplot(data.frame(Observed = dat2$log_antibody, Predicted = pred_mars), 
  ...
  labs(
    title = "Predicted vs. Observed Antibody Levels",
    x = "Observed Log Antibody Level",
    y = "Predicted Log Antibody Level"
  ) +
  theme_minimal()

residuals <- dat2$log_antibody - pred_mars

ggplot(data.frame(Predicted = pred_mars, Residuals = residuals), 
       aes(x = pred_mars, y = residuals)) +
